...
prop_readBackTicked3 = isWarning (readBackTicked False) "Â´grep \"\\\"\"Â´"
...
readBackTicked quoted = called "backtick expansion" $ do
    ...
    subString <- readGenericLiteral "`Â´"
    endPos <- getPosition
    backtick
    id <- endSpan start
    ...
    backtick =
      void (char '`') <|> do
         ...
         parseProblemAt pos ErrorC 1077
            "For command expansion, the tick should slant left (` vs Â´). Use $(..) instead."
