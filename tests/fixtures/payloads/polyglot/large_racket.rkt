       ...
       (let ([key (or key (Î» (x) x))])
         (let-syntax ([loop (syntax-rules ()
           ...
           (cond [(eq? =? equal?) (loop member)]
                 ...
                 [else (loop (Î»(x seen) (ormap (Î»(y) (=? x y)) seen)))])))]
      [else
...
(define (check-duplicates items
  ...
  (let ([fail-k (if (procedure? failure-result) failure-result (Î» () failure-result))])
    (cond [(eq? same? equal?)
         ...
         (define (next)
           ...
           (cond [(<= ci i) (bytes-set! c i (add1 ci))
                 ...
                 [(< i N)   (bytes-set! c i 0)
                            ...
                            (next)]
                 [else      #f]))
         (in-producer (Î»() (begin0 cur (set! cur (next)))) #f)]))
