...
it can return `{continue,Â Continue}` in place of
...
- **`{noreply,NewState}`\
  ...
  [`reply(From,Â Reply)`](`reply/2`), either in this
...
- **`{stop,Reason,NewState}`\
  ...
  [`reply(From,Â Reply)`](`reply/2`) before returning `{stop,_,_}`.
""".
...
`{continue,Â Continue}`.  The call is invoked immediately after
...
when the instruction `{update,Â Module,Â Change,Â ...}`, is specified
...
> #### Note {: .info }
>
> If a release upgrade/downgrade with `ChangeÂ =Â {advanced,Â Extra}`
...
`[{data,Â [{"State",Â Term}]}]`, where `Term` provides relevant details
...
- **`TimeÂ ::Â `[`timeout()`](`t:timeout/0`)** -
  ...
  For `TimeÂ =Â 0`, if there is a request or message waiting to be received,
  ...
  > `{timeout|hibernate,Â ...}` time-outs described below,
  > so it is recommended to not use them
  > in combination with this legacy time-out type.

- **`{timeout,Â Time,Â Message}`\
  `{timeout,Â Time,Â Message,Â Options}`** - Like `Time` above,
  ...
  A relative time-out with `TimeÂ =Â 0` is immediately delivered,
  before any request or message is received, including system messages.

  A time-out with `TimeÂ =Â infinity` will never be delivered
  so the action is ignored; no time-out is started.

- **`{hibernate,Â Time,Â Message}`\
  `{hibernate,Â Time,Â Message,Â Options}`** - A combination of
  the action `hibernate` below, and  `{timeout,Â Time,Â Message}`
  or `{timeout,Â Time,Â Message,Â Options}` above.
...
- **`{continue,Â Continue}`** - The process will immediately execute the
...
- **`{local,Â LocalName}`** - Register the `gen_server` locally
  as `LocalName` using [`register/2`](`erlang:register/2`).

- **`{global,Â GlobalName}`** - Register the `gen_server` process id
  globally as `GlobalName` using `global:register_name/2`.

- **`{via,Â RegMod,Â ViaName}`** - Register the `gen_server` process
  ...
  Thus, `{via,Â global,Â GlobalName}` is a valid reference
  equivalent to `{global,Â GlobalName}`.
""".
...
- **`{global,Â GlobalName}`** - The `gen_server` is globally registered
  in `m:global`.

- **`{via,Â RegMod,Â ViaName}`** - The `gen_server` is registered
...
- **`{timeout,Â Timeout}`** - How many milliseconds
  the `gen_server` process is allowed to spend initializing
  or it is terminated and the start function returns `{error,Â timeout}`.

- **`{spawn_opt,Â SpawnOptions}`** - The `SpawnOptions` option list
...
- **`{hibernate_after,Â HibernateAfterTimeout}`** - Specifies that the
...
- **`{debug,Â Dbgs}`** - For every entry in `Dbgs`,
...
- **`{ok,Â Pid}`** - The `gen_server` process was succesfully created and
  initialized, with the process identifier `Pid`.

- **`{error,Â {already_started,Â OtherPid}}`** - A process with the specified
...
- **`{error,Â timeout}`** - The `gen_server` process failed to initialize
...
If `start_link/3,4` returns `ignore` or `{error,Â _}`,
...
> `{stop,Â Reason}` from [`Module:init/1`](`c:init/1`), this function
> could return `{error,Â Reason}` _before_ the started `m:gen_server` process
...
> [`Module:init/1`](`c:init/1`), a process link `{'EXIT',Â Pid,Â Reason}`
> message caused this function to return `{error,Â Reason}`,
...
The difference between returning `{stop,Â _}` and `{error,Â _}` from
[`Module:init/1`](`c:init/1`), is that `{error,Â _}` results in a graceful
...
process with an exit term containing `ReasonÂ =Â timeout` as described below.
...
> of the form `{reference(),Â _}` and deal with them appropriately
...
`{Reason,Â Location}` where `LocationÂ =Â {gen_server,Â call,Â ArgList}`
...
The call
`gen_server:receive_response(gen_server:send_request(ServerRef, Request),Â Timeout)`
can be seen as equivalent to
[`gen_server:call(ServerRef, Request,Â Timeout)`](`call/3`),
...
in a 3-tuple `{Response,Â Label,Â NewReqIdCollection}`.
...
Returns a list of `{ReqId,Â Label}` tuples which corresponds to
...
[`Module:handle_cast(Request,Â _)`](`c:handle_cast/2`)
to handle the request.
""".
-spec cast(
        ServerRef :: server_ref(),
...
The function returns a tuple `{Replies,Â BadNodes}`,
where `Replies` is a list of `{Node,Â Reply}` tuples,
...
[`enter_loop(Module,Â Options,
  State,Â ServerName,Â infinity)`](`enter_loop/5`).

With argument `Action` equivalent to
[`enter_loop(Module,Â Options, State,Â self(),Â Action)`](`enter_loop/5`).
""".
-spec enter_loop(
        Module     :: module(),
        Options    :: [enter_loop_opt()],
