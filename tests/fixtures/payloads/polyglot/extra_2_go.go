...
func (c *conn) closeWriteAndWait() {
	...
	// that â€œ[t]he server â€¦ continues to read from the connection until it
	...
	// acknowledgement of the packet(s) containing the server's last response.â€
	//
	// Unfortunately, we have no straightforward way to be â€œreasonably certainâ€
	...
	//
	// Instead, we declare that we are â€œreasonably certainâ€ that we received the
...
// sets Content-Type to â€œtext/plain; charset=utf-8â€,
// and sets X-Content-Type-Options to â€œnosniffâ€.
...
// that replies to each request with a â€œ404 page not foundâ€ reply.
...
// Handler returns a â€œpage not foundâ€ or â€œmethod not supportedâ€
