...
//!
//! Direct XML parsing from PPTX (ZIP) for clean text
...
impl PptxConverter {
    ...
    /// Extract text directly from PPTX XML
...
impl DocumentConverter for PptxConverter {
        ...
        match output_format {
            ...
            _ => {
                return Err(crate::TransmutationError::InvalidOptions(format!(
                    "Unsupported output format for PPTX: {:?}",
                    output_format
                )));
            }
        }
    ...
    fn metadata(&self) -> ConverterMetadata {
        ConverterMetadata {
            ...
            description: "PPTX converter: Direct XML parsing (Markdown)".to_string(),
