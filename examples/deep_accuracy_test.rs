...
impl BoringTable {
    ...
    fn is_needle(&self, token: &str) -> bool {
        ...
        clean.contains('@') || clean.contains('_') || clean.contains(':') || (is_mixed && !clean.chars().next().unwrap().is_uppercase()) || is_id || (is_mixed && upper_count > 1)
    }
}
...
fn generate_noise(lines: usize) -> String {
    ...
    for i in 0..lines {
        out.push_str(&format!("// JUNK_LICENSE_HEADER_LINE_{}: boilerplate text that means nothing to the agent\n", i));
    }
    out
}
...
fn calculate_proximity_score(source: &str, crushed: &str, needles: &[String]) -> f64 {
    ...
    if needles.is_empty() { return 1.0; }
...
fn main() {
    ...
    println!("{:<25} | {:>8} | {:>8} | {:>8} | Accuracy", "Asset File", "InSize", "OutSize", "Comp %");
    println!("{:-<85}", "");
    ...
    for entry in assets.filter_map(|e| e.ok()) {
        ...
        for needle in &needles {
            if !output.contains(needle) { missed.push(needle); }
        }
        
        let signal_retention = if needles.is_empty() { 1.0 } else {
        ...
        let status = if cas >= 90.0 { "\x1b[32mPASS\x1b[0m" } else if cas >= 70.0 { "\x1b[33mWARN\x1b[0m" } else { "\x1b[31mFAIL\x1b[0m" };
        
        println!("{:<25} | {:>8} | {:>8} | {:>7.1}% | {:.1}% {}", 
            filename, mutated_input.len(), output.len(), comp_ratio * 100.0, cas, status);
